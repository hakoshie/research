...
\begin{figure}[H]
    ...
    \end{minipage}
\end{figure}
両方ともプレトレンドはないようである。OTCの出荷額は、販売開始後、最初の数年はおよそ35\%\((=\exp(0.3)-1)\)程度増えていて、その後はおよそ65\%\((=\exp(0.5)-1)\)程度増えている。医療用医薬品の出荷額については、販売開始前後でほとんど変化がなく、推定値もほとんど0の近くにある。
...
価格弾力性は次のように定義されたメソッドで計算してある\footnote{\url{https://pyblp.readthedocs.io/en/stable/_api/pyblp.ProblemResults.compute_elasticities.html}}。
\begin{align*}
...
消費者余剰は次のように定義されたメソッドで計算してある\footnote{\url{https://pyblp.readthedocs.io/en/latest/_api/pyblp.ProblemResults.compute_consumer_surpluses.html}}。
\begin{align*}
