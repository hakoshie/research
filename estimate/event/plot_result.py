...
def plot_result(result, start_from=1, period=17, insert_index=6, time_start=-7,ylim=(-1,1),plot_type="o-",file_name="result", title="Event Study Coefficients with Confidence Intervals",markersize=5,color_palette="Paired"):
    ...
    plt.plot(summary_table['Time'], summary_table["Parameter"], plot_type, label='Coefficients', color=colors[1],markersize=markersize)

    # Plot confidence intervals as error bars
    plt.fill_between(summary_table['Time'], summary_table['Lower CI'], summary_table['Upper CI'], color=colors[0], alpha=0.3, label='Confidence Intervals')
